...
from agent2.planner       import create_plan, replan
from agent2.error_handler import analyze_error, generate_fix, ErrorDecision
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
        from backend.action.open_app import open_app
        return open_app(parameters=parameters, player=None) or "Done."

    elif tool == "web_search":
        from backend.action.web_search import web_search
        return web_search(parameters=parameters, player=None) or "Done."

    elif tool == "game_updater":
        from backend.action.game_updater import game_updater
...
class AgentExecutor:
    ...
    def execute(
        ...
        completed_steps = []
        step_results    = {}
        plan            = create_plan(goal, speak=effective_speak)
        ...
        while True:
            ...
            if not steps:
                ...
                if effective_speak: effective_speak(msg)
                return msg
            ...
            for step in steps:
                if cancel_flag and cancel_flag.is_set():
                    if effective_speak: effective_speak("Task cancelled, sir.")
                ...
                while attempt <= 3:
                    if cancel_flag and cancel_flag.is_set():
                        break
                    try:
                        result = _call_tool(tool, params, effective_speak)
                        ...
                        recovery = analyze_error(step, error_msg, attempt=attempt, speak=effective_speak)
                        decision = recovery["decision"]
                        user_msg = recovery.get("user_message", "")

                        if effective_speak and user_msg:
                            effective_speak(user_msg)
                        ...
                        if decision == ErrorDecision.RETRY:
                            ...
                            if fix_suggestion and tool != "generated_code":
                                try:
                                    fixed_step = generate_fix(step, error_msg, fix_suggestion, speak=effective_speak)
                                    if effective_speak: effective_speak("Trying an alternative approach, sir.")
                                    res = _call_tool(
                                        fixed_step["tool"],
            ...
            if success:
                return self._summarize(goal, completed_steps, effective_speak)
            ...
            if effective_speak: effective_speak("Adjusting my approach, sir.")

            replan_attempts += 1
            plan = replan(goal, completed_steps, failed_step, failed_error, speak=effective_speak)
