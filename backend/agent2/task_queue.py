...
class TaskQueue:
    def __init__(self, max_concurrent: int = 1, speak: Optional[Callable] = None):
        ...
        self._executor       = None
        self._default_speak  = speak

    def _get_executor(self):
        if self._executor is None:
            from agent2.executor import AgentExecutor
            self._executor = AgentExecutor(speak=self._default_speak)
    ...
    def start(self) -> None:
        ...
        if self._default_speak:
            self._default_speak("Task queue started, sir.")
    ...
    def stop(self) -> None:
        ...
        if self._default_speak:
            self._default_speak("Task queue stopped, sir.")
    ...
    def submit(
        ...
        effective_speak = speak or self._default_speak
        task    = Task(
            priority    = priority.value,
            created_at  = time.time(),
            task_id     = task_id,
            goal        = goal,
            speak       = effective_speak,
        ...
        if effective_speak:
            effective_speak(f"Task queued with ID {task_id}, sir.")
        return task_id
    ...
    def cancel(self, task_id: str) -> bool:
        with self._lock:
            ...
            if self._default_speak:
                self._default_speak(f"Task {task_id} cancelled, sir.")
            return True
    ...
    def _run_task(self, task: Task) -> None:
        ...
        try:
            ...
            if task.speak:
                task.speak(f"Task {task.task_id} failed, sir.")
...
def get_queue(speak: Optional[Callable] = None) -> TaskQueue:
    global _queue_started, _queue
    with _queue_lock:
        if not _queue_started:
            if speak:
                _queue = TaskQueue(speak=speak)
            _queue.start()
            _queue_started = True
    return _queue
