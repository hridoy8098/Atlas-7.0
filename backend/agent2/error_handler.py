...
class ErrorAnalyzer:
    ...
    def analyze(
        ...
        return analyze_error(step, error, attempt, max_attempts, self.speak)

    def generate_fix(self, step: dict, error: str, fix_suggestion: str) -> dict:
        return generate_fix(step, error, fix_suggestion, self.speak)
...
def analyze_error(
    ...
    if attempt >= max_attempts:
        msg = f"Max attempts reached for step {step.get('step')} — forcing replan"
        print(f"[ErrorHandler] {msg}")
        if speak:
            speak("Maximum attempts reached, trying a different approach, sir.")
        return {
...
Attempt number: {attempt}"""
    ...
    try:
        ...
        if step.get("critical") and result["decision"] == ErrorDecision.SKIP:
            ...
            if speak:
                speak("This step is critical, finding an alternative approach, sir.")
        ...
        if speak:
            speak("Encountered an issue, adjusting approach, sir.")
        return {
            "decision":       ErrorDecision.REPLAN,
            "reason":         str(e),
...
Return ONLY the Python code, no explanation."""
    ...
    try:
        ...
        if speak:
            speak("Generating a fix for this issue, sir.")
