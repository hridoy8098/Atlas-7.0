...
def create_plan(
    ...
    try:
        ...
        if speak:
            speak(f"Creating a plan with {len(plan['steps'])} steps, sir.")
        ...
        return _fallback_plan(goal, speak)
...
def _fallback_plan(
    ...
    if speak:
        speak("Using a fallback plan for this task, sir.")
    return {
        "goal": goal,
        "steps": [
            {
...
Create a REVISED plan for the remaining work only. Do not repeat completed steps."""
    ...
    try:
        ...
        if speak:
            speak("Revised plan created, sir.")
